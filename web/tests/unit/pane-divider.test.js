...
test('defaults: 220 px minimum, 60% of the window maximum, 260 / 670 per tab', () => {
  assert.equal(MIN_PANE_WIDTH, 220);
  assert.equal(MAX_PANE_FRACTION, 0.6);
  assert.deepEqual(DEFAULT_PANE_WIDTHS, { lights: 260, rig: 670 });
...
test('resolveTabWidth uses a finite stored number, else the tab default', () => {
  ...
  assert.equal(resolveTabWidth(undefined, 'rig'), 670);
  assert.equal(resolveTabWidth('', 'rig'), 670);
  assert.equal(resolveTabWidth('abc', 'lights'), 260);
  assert.equal(resolveTabWidth('NaN', 'rig'), 670);
  assert.equal(resolveTabWidth('Infinity', 'rig'), 670);
